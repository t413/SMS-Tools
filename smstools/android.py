...
class Android:
    ...
    def parse(self, filepath):
        """Parse a sqlite file from android format to Text[] """
	...
	"""Gets content of the android db."""
        texts = []
        ...
        for row in query:
            ...
            txt = core.Text(num=row[0],date=long(row[1]),date_sent=long(row[2]),incoming=(row[3]==2),body=row[4])
            texts.append(txt)
        return texts

    def write(self, texts, outfilepath):
        """Write a Text[] to sqlite file into android format."""
        print "Creating empty Android SQLITE db"
        conn = sqlite3.connect(outfilepath)
        cursor = conn.cursor()
        cursor.executescript(INIT_DB_SQL)
	...
	# associate a phone number/mail to its _id as following contactIdFromNumber[phoneNumber]=_id
        contactIdFromNumber = {}
        ...
        for txt in texts:
            ...
            if False: ## TODO move this debug output to better better comments
                ...
                print "adding entry to message db: " + str([txt.num,txt.date,txt.date,txt.body,thread_id,txt.incoming+1])
            ...
            cursor.execute( "INSERT INTO sms (address,date,date_sent,body,thread_id,read,type,seen) VALUES (?,?,?,?,?,1,?,1)", [txt.num, txt.date, txt.date, txt.body, thread_id, txt.incoming+1])
...
#for each threads._id=sms.thread_id, set threads.read = 0 if a sms is not read, 1 else
